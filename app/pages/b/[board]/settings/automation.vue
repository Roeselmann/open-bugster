...
/**
 * Which webhook is being edited, and the draft it holds.
 *
 * Editing in place rather than through remove-and-recreate: the signing secret is shown once,
 * so recreating a webhook to change one event means reconfiguring the receiver as well.
 */
const editing = ref<string | null>(null)
const saving = ref(false)
const draft = reactive({ url: '', description: '', events: [] as string[] })

function edit(hook: WebhookRecord) {
  if (editing.value === hook.id) {
    editing.value = null
    return
  }
  draft.url = hook.url
  draft.description = hook.description
  draft.events = [...hook.events]
  editing.value = hook.id
}

async function save(hook: WebhookRecord) {
  if (!draft.url.trim() || !draft.events.length || saving.value) return
  saving.value = true
  try {
    await $fetch(`/api/webhooks/${hook.id}`, {
      method: 'PATCH',
      body: { url: draft.url.trim(), description: draft.description.trim(), events: [...draft.events] },
    })
    editing.value = null
    notify('success', 'The webhook was updated.')
    await load()
  } catch (error) {
...
async function remove(hook: WebhookRecord) {
  try {
    ...
    if (editing.value === hook.id) editing.value = null
    await load()
  } catch (error) {
    notify('error', errorText(error))
  }
    ...
    <section class="surface rounded-2xl">
      <header class="border-b border-[var(--line)] px-5 py-4">
        ...
        <p class="muted mt-1 text-sm">
          ...
          What gets sent is written out below.
        </p>
      </header>
      ...
      <div class="space-y-5 px-5 py-5">
        ...
        <form class="space-y-4" @submit.prevent="create">
          ...
          <fieldset>
            ...
            <WebhookEventPicker v-model="form.events" />
          </fieldset>
        ...
        <div v-if="!loading" class="space-y-2 border-t border-[var(--line)] pt-4">
          <div v-for="hook in hooks" :key="hook.id" class="surface-strong rounded-xl px-3 py-2.5">
            ...
            <p v-if="editing !== hook.id" class="muted mt-1 text-[11px]">
              <span class="mr-1.5 font-bold uppercase tracking-[.08em]">Sends</span>
              <span class="font-mono">{{ hook.events.join(' · ') }}</span>
            </p>

            <form v-if="editing === hook.id" class="mt-3 space-y-3 border-t border-[var(--line)] pt-3" @submit.prevent="save(hook)">
              <div class="grid gap-3 sm:grid-cols-2">
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[.08em]">Where to send it</span>
                  <input v-model="draft.url" required type="url" class="focus-ring surface rounded-xl h-10 w-full px-3 text-sm outline-none">
                </label>
                <label class="block">
                  <span class="mb-1.5 block text-xs font-bold uppercase tracking-[.08em]">What it is</span>
                  <input v-model="draft.description" maxlength="200" placeholder="n8n — triage workflow" class="focus-ring surface rounded-xl h-10 w-full px-3 text-sm outline-none">
                </label>
              </div>

              <fieldset>
                <legend class="mb-2 block text-xs font-bold uppercase tracking-[.08em]">When to send it</legend>
                <WebhookEventPicker v-model="draft.events" />
              </fieldset>

              <div class="flex flex-wrap items-center justify-end gap-3">
                <p class="muted mr-auto text-xs">The signing secret and the delivery log stay as they are.</p>
                <button type="button" class="focus-ring muted h-9 rounded-xl px-3 text-sm font-semibold hover:text-[var(--ink)]" @click="editing = null">Cancel</button>
                <button
                  :disabled="saving || !draft.url.trim() || !draft.events.length"
                  class="focus-ring h-9 rounded-xl bg-[var(--ink)] px-4 text-sm font-semibold text-[var(--canvas)] transition hover:opacity-85 disabled:opacity-50"
                >
