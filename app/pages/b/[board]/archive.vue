    ...
    <main class="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <div v-else class="mt-8 space-y-3">
        <article v-for="ticket in tickets" :key="ticket.id" class="surface-strong flex items-center gap-4 rounded-2xl p-4 sm:p-5">
          ...
          <button v-if="canEdit" :disabled="restoring === ticket.id" class="focus-ring flex h-10 shrink-0 items-center gap-2 rounded-xl border border-[var(--line)] px-3 text-sm font-semibold hover:bg-[var(--panel)] disabled:opacity-50" @click="restore(ticket)"><ArchiveRestore :size="16" /> <span class="hidden sm:inline">Restore</span></button>
        </article>
      </div>
    </main>
  </div>
