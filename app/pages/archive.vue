...
import { ArchiveRestore, Inbox, Tag } from '@lucide/vue'
import type { Ticket } from '~~/shared/types/domain'

const { data, pending, refresh } = await useFetch<{ tickets: Ticket[] }>('/api/tickets?archived=true')
...
<template>
  <div class="min-h-screen">
    <AppHeader :syncing="false" :latest-run="null" archive-mode @logout="logout" />
    <main class="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <p class="muted mb-1 text-[11px] font-bold uppercase tracking-[.14em]">Removed from the board</p>
      <h1 class="text-3xl font-bold tracking-[-.045em]">Archive</h1>
      <p class="muted mt-2 text-sm">Archived TestFlight tickets are not imported again during a sync.</p>
