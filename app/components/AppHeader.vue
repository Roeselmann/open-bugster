...
import { Archive, Bug, LogOut, Moon, RefreshCcw, Sun } from '@lucide/vue'
import type { SyncRun } from '~~/shared/types/domain'

const props = defineProps<{ boardId: string; syncing: boolean; latestRun: SyncRun | null; archiveMode?: boolean }>()
const emit = defineEmits<{ sync: []; logout: [] }>()
...
<template>
  <header class="border-b border-[var(--line)] bg-[color-mix(in_srgb,var(--panel)_88%,transparent)] backdrop-blur-xl">
    <div class="mx-auto flex min-h-18 max-w-[1800px] items-center gap-3 px-4 sm:px-6">
      <NuxtLink :to="`/b/${props.boardId}`" class="focus-ring mr-auto flex items-center gap-3 rounded-xl" aria-label="Open-Bugster board">
      ...
      <NuxtLink
        :to="archiveMode ? `/b/${props.boardId}` : `/b/${props.boardId}/archive`"
